...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/LLM-Claude%20(Anthropic)-d97706?logo=anthropic&logoColor=white" alt="Claude"/>
...
DataPilot 是一个基于 **LangGraph + Claude** 的智能数据分析 Agent。用户只需用自然语言描述分析需求，Agent 就会自动完成以下全流程：
...
| 层级 | 技术 | 选择理由 |
|------|------|----------|
| **LLM** | Anthropic Claude (Sonnet 4.6 / Opus 4.8) | 强大的 SQL 生成和推理能力 |
...
| **可视化** | Matplotlib | 成熟的绑图库，支持非交互式后端 (Agg) |
...
- **模型选择** — 在 Sonnet 4.6 / Opus 4.8 / Haiku 4.5 之间切换
- **数据库状态** — 实时显示数据表名和行数
- **操作按钮** — 重新生成示例数据 / 清空对话
...
python main.py ask --model claude-opus-4-8 "复杂分析问题"
```
...
- **Planner** — 使用 `with_structured_output(AnalysisPlan)` 将模糊问题拆解为 2-5 个原子步骤
- **Executor** — 使用 `bind_tools(ALL_TOOLS)` 让 LLM 选择合适的工具执行每一步；失败时自动将错误反馈给 LLM 重试（最多 2 次）
- **Evaluator** — 使用 `with_structured_output(EvaluationResult)` 判断是否已完整回答；若不足则生成 `additional_steps` 触发重新规划
...
<p align="center">
  <sub>Built with ❤️ using LangGraph + Anthropic Claude + DuckDB + Streamlit</sub>
</p>
#   D a t a P i l o t  
 